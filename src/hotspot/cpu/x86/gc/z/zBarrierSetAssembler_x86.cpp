...
void ZBarrierSetAssembler::store_barrier_fast(MacroAssembler* masm,
  ...
  if (in_nmethod) {
    ...
    if (rnew_zaddress != noreg) {
      ...
      __ Assembler::orq(rnew_zpointer, barrier_Relocation::unpatched);
...
void ZBarrierSetAssembler::copy_at(MacroAssembler* masm,
  ...
  // Color
  __ Assembler::orq(tmp1, (int32_t)(uint32_t)ZPointerStoreGoodMask);
...
void ZBarrierSetAssembler::try_resolve_jobject_in_native(MacroAssembler* masm,
  ...
  // Test for weak tag
  __ testptr(obj, JNIHandles::weak_tag_mask);
...
static void z_color(LIR_Assembler* ce, LIR_Opr ref) {
  ...
  __ Assembler::orq(ref->as_register(), barrier_Relocation::unpatched);
  __ relocate(barrier_Relocation::spec(), ZBarrierRelocationFormatStoreGoodAfterOr);
}
