...
void G1BarrierSetAssembler::g1_write_barrier_pre(MacroAssembler* masm,
  ...
  if (from_c2) {
    ...
    __ call(RuntimeAddress(CAST_FROM_FN_PTR(address, G1BarrierSetRuntime::write_ref_field_pre_entry)));
  } else if (expand_call) {
...
void G1BarrierSetAssembler::g1_write_barrier_post(MacroAssembler* masm,
...
#ifdef _LP64
  __ movslq(store_addr, queue_index);
  __ addq(tmp2, store_addr);
  ...
  if (from_c2) {
    if (c_rarg0 != card_addr) {
      __ mov(c_rarg0, card_addr);
    }
    __ mov(c_rarg1, thread);
    __ call(RuntimeAddress(CAST_FROM_FN_PTR(address, G1BarrierSetRuntime::write_ref_field_post_entry)));
  } else {
#ifdef _LP64
