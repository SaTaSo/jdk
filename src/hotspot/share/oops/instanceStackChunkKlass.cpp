...
size_t InstanceStackChunkKlass::uncompressed_oop_size(oop obj) const {
  // see oopDesc::size_given_klass
  return instance_size(jdk_internal_vm_StackChunk::size(obj));
}

size_t InstanceStackChunkKlass::oop_size(oop obj) const {
  return TrimContinuationChunksInGC ? compact_oop_size(obj) : uncompressed_oop_size(obj);
}
...
#ifdef ASSERT
  size_t old_compact_size = obj->size();
  size_t old_size = static_cast<InstanceStackChunkKlass*>(obj->klass())->uncompressed_oop_size(obj);
