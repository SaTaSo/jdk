...
public:
  ...
  void do_forwarding(ZForwarding* forwarding) {
    ...
    ZPage*& target = _target[static_cast<uint>(_forwarding->to_age()) - 1];
    ...
    if (in_place) {
      ...
      _allocator->share_target_page(target);
    } else {
...
ZPageAge ZRelocate::compute_to_age(ZPageAge from_age, bool promote_all) {
  if (promote_all || from_age == ZPageAge::old) {
    return ZPageAge::old;
  } else {
    uint age = static_cast<uint>(from_age);
    if (age > InitialTenuringThreshold) {
      return ZPageAge::old;
    }
