...
void ZArguments::select_max_gc_threads() {
  ...
  uint default_nconcurrent = ZHeuristics::nconcurrent_workers();
  ...
  if (FLAG_IS_DEFAULT(ZYoungGCThreads)) {
    if (UseDynamicNumberOfGCThreads) {
      FLAG_SET_ERGO(ZYoungGCThreads, default_nconcurrent);
    } else {
      uint static_young_threads = MAX2(uint(default_nconcurrent * 0.9), 1u);
  ...
  if (FLAG_IS_DEFAULT(ZOldGCThreads)) {
    if (UseDynamicNumberOfGCThreads) {
      FLAG_SET_ERGO(ZOldGCThreads, default_nconcurrent);
    } else {
      uint static_old_threads = MAX2(ConcGCThreads - ZYoungGCThreads, 1u);
