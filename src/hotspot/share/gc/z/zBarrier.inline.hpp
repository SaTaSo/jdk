...
inline zpointer color_mark_young_from_old_good(zaddress new_addr, zpointer old_ptr) {
...
inline void ZBarrier::mark_barrier_on_old_oop_field(volatile zpointer* p, bool finalizable) {
  assert(ZHeap::heap()->is_old(p), "Should be from young");
  const zpointer o = load_atomic(p);
...
inline zaddress ZBarrier::mark_young_good_barrier_on_old_oop_field(volatile zpointer* p) {
  const zpointer o = load_atomic(p);
  return barrier(is_mark_young_good_fast_path, mark_young_slow_path, color_mark_young_from_old_good, p, o);
}
