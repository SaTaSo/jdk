...
size_t ZStatHeap::allocated(size_t used_generation, size_t reclaimed) {
  ...
  return (used_generation + reclaimed) - _at_mark_start.used_generation;
}
...
void ZStatHeap::set_at_mark_end(const ZPageAllocatorStats& stats) {
  ...
  _at_mark_end.allocated = allocated(stats.used_generation(), 0 /* reclaimed */);
}
...
void ZStatHeap::set_at_relocate_start(const ZPageAllocatorStats& stats) {
  ...
  _at_relocate_start.allocated = allocated(stats.used_generation(), stats.reclaimed());
...
void ZStatHeap::set_at_relocate_end(const ZPageAllocatorStats& stats, size_t non_worker_relocated) {
  ...
  _at_relocate_end.allocated = allocated(stats.used_generation(), reclaimed);
...
void ZStatHeap::print(ZCollector* collector) {
  ...
  log_info(gc, heap)("%s", heap_table()
                     ...
                     .end());
  log_info(gc, heap)("%s statistics:", collector->id() == ZCollectorId::_major ? "Old generation" : "Young generation");
