...
size_t ZStatHeap::allocated(size_t used_generation, size_t reclaimed, size_t relocated) {
  ...
  return (used_generation + reclaimed) - _at_mark_start.used_generation - relocated;
}
...
void ZStatHeap::set_at_relocate_start(const ZPageAllocatorStats& stats) {
  ...
  _at_relocate_start.allocated = allocated(stats.used_generation(), stats.reclaimed(), stats.relocated());
  _at_relocate_start.garbage = garbage(stats.reclaimed(), stats.relocated(), stats.promoted());
  _at_relocate_start.reclaimed = stats.reclaimed() - stats.promoted() - stats.relocated();
...
void ZStatHeap::set_at_relocate_end(const ZPageAllocatorStats& stats, size_t non_worker_relocated, size_t non_worker_promoted) {
  ...
  _at_relocate_end.reclaimed = stats.reclaimed() - promoted - relocated;
