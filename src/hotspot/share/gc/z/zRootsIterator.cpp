...
public:
  ...
  static const ZStatSubPhase* calculate_subphase(const ZGeneration* generation, const ZRootStatSubPhase& subphase) {
    if (generation == NULL) {
      return NULL;
    }

    if (generation->is_young()) {
      return &subphase.young();
    }
...
public:
  ZRootStatTimer(const ZRootStatSubPhase& subphase, const ZGeneration* generation) :
...
ZNMethodsIteratorImpl::ZNMethodsIteratorImpl(ZGeneration* generation, bool enabled, bool secondary) :
    _enabled(enabled),
