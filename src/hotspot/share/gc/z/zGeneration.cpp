...
uint ZGenerationYoung::compute_tenuring_threshold(ZRelocationSetSelectorStats stats) {
  ...
  log_info(gc, reloc)("Young Allocated: " SIZE_FORMAT "M", young_allocated / M);
  log_info(gc, reloc)("Young Garbage: " SIZE_FORMAT "M", young_garbage / M);
  log_info(gc, reloc)("Allocated To Garbage: %.1f", allocated_garbage_ratio);
  log_info(gc, reloc)("Young Log: %.1f", young_log);
  log_info(gc, reloc)("Young Residency Reciprocal: %.1f", young_residency_reciprocal);
  log_info(gc, reloc)("Young Residency Factor: %.1f", young_residency_factor);
  ...
  uint tenuring_threshold = clamp((uint)round(tenuring_threshold_raw), 1u, last_populated_age + 1u);
