...
static ZStoreBarrierBufferTable* store_barrier_buffer_table = NULL;

#define BAD_REMSET_ARG(p, ptr, o) \
  "Missing remembered set at " PTR_FORMAT " pointing at " PTR_FORMAT \
  " (" PTR_FORMAT " + " INTX_FORMAT ")" \
  , p2i(p), untype(ptr), o, p2i(p) - o
...
public:
  ...
  void set_obj_addr(uintptr_t addr) {
    _obj_addr = addr;
  }

  virtual void do_oop(oop* p_) {
    volatile zpointer* p = (zpointer*)p_;
    zpointer ptr = *p;
    ...
    if (ZBufferStoreBarriers && store_barrier_buffer_table->get(p) != NULL) {
      // If this oop location is in the store barrier buffer, we can't assume
      // that it should have a remset entry
      return;
    }

    if (_forwarding->find(to_zaddress_unsafe(_obj_addr)) != zaddress::null) {
...
void ZVerify::on_color_flip() {
  ...
  delete store_barrier_buffer_table;
  store_barrier_buffer_table = new (ResourceObj::C_HEAP, mtGC) ZStoreBarrierBufferTable();
  ...
  for (JavaThreadIteratorWithHandle jtiwh; JavaThread* jt = jtiwh.next(); ) {
    ...
    for (int i = buffer->current(); i < (int)ZStoreBarrierBuffer::_buffer_length; ++i) {
      ...
      bool created = false;
      store_barrier_buffer_table->put_if_absent(p, true, &created);
    }
  }
}
...
private:
  ZForwarding* _forwarding;
  uintptr_t _fromspace_obj_addr;
  uintptr_t _tospace_obj_addr;

public:
  ZVerifyRemsetAfterOopClosure(ZForwarding* forwarding) :
      _forwarding(forwarding) {}

  void set_fromspace_obj_addr(uintptr_t addr) {
    _fromspace_obj_addr = addr;
  }

  void set_tospace_obj_addr(uintptr_t addr) {
    _tospace_obj_addr = addr;
  }
  ...
  virtual void do_oop(oop* p_) {
    ...
    uintptr_t p_offset = ((uintptr_t)p) - _tospace_obj_addr;
    volatile zpointer* fromspace_p = (volatile zpointer*)(_fromspace_obj_addr + p_offset);

    if (ZBufferStoreBarriers && store_barrier_buffer_table->get(fromspace_p) != NULL) {
    ...
    if (!page->is_remembered(p) && !page->was_remembered(p)) {
      guarantee(ZGeneration::young()->is_phase_mark(), "Should be in the mark phase " BAD_REMSET_ARG(p, ptr, _tospace_obj_addr));
      guarantee(_forwarding->relocated_remembered_fields_published_contains(p), BAD_REMSET_ARG(p, ptr, _tospace_obj_addr));
    }
  }
...
void ZVerify::after_relocation_internal(ZForwarding* forwarding) {
  ...
  forwarding->address_unsafe_iterate_via_table([&](zaddress_unsafe fromspace_addr) {
    // If no field in this object was in the store barrier buffer
    // when relocation started, we should be able to verify trivially
    ZGeneration* remap_generation = forwarding->from_age() == ZPageAge::old ? (ZGeneration*)ZGeneration::old()
                                                                            : (ZGeneration*)ZGeneration::young();
    zaddress tospace_addr = remap_generation->remap_object(fromspace_addr);
    if (ZAbort::should_abort()) {
      // We can't verify remembered set accurately when shutting down the VM
    }
    cl.set_fromspace_obj_addr(untype(fromspace_addr));
    cl.set_tospace_obj_addr(untype(tospace_addr));
    oop tospace_obj = to_oop(tospace_addr);
    tospace_obj->oop_iterate(&cl);
  });
}
