...
ZDriverRequest rule_minor_allocation_rate_dynamic(double serial_gc_time_passed, double parallel_gc_time_passed) {
  ...
  return ZDriverRequest(GCCause::_z_minor_allocation_rate, actual_gc_workers, 0);
}
...
static uint rule_major_allocation_rate_threads() {
  ...
  for (uint i = 2; i <= ConcGCThreads; ++i) {
    ...
    double potential_reduced_old_duration = potential_old_duration - baseline_old_duration;
    double potential_reduced_yc_count = potential_old_duration / young_gc_duration;
    double reduced_extra_young_gc_time = potential_reduced_yc_count * extra_young_gc_time;
    double extra_old_gc_time = extra_threads * old_parallelizable_gc_time;
