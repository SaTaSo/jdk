 ...
 public:
      ...
      _only_not_unloading(filter == only_alive_and_not_unloading)
  {
    if (is_compiled_method) {
      CodeCache::begin_compiled_method_iteration();
  ...
  ~CodeBlobIterator() {
    if (is_compiled_method) {
      CodeCache::end_compiled_method_iteration();
    }
  }
...
typedef CodeBlobIterator<CompiledMethod, CompiledMethodFilter, true /* is_compiled_method */> BetterCompiledMethodIterator;
